...
\end{lemma}
\begin{defn}[concave function]
	Let $f$ be a real valued function defined on [a,b]. $f$ is said to be concave iff $\forall x_1,x_2 \in [a,b]$ and $\forall \lambda \in [0,1]$
	\begin{align*}
...
\begin{thm}[the Kullback-Leibler divergence is always non-negative.]
	\begin{align*}
		\KLD{q(z)}{p(z)} \ge 0
	\end{align*}
\end{thm}
\begin{proof}
	$\log(x)$ is a concave function, so $-\log(x)$ is convex. 
	\begin{align*}
		\KLD{q(z)}{p(z)} = & \Expectation_q(\log{\frac{q(z)}{p(z)}})\\
        = & \Expectation_q(-\log{\frac{p(z)}{q(z)}})\\
         \ge &\log{\Expectation_q(\frac{p(z)}{q(z)})} \text{, now}\\
