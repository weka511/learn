...
\end{align*}
Since, as \cite{blei2017variational} notes, this is generally intractable, we will use the Kullback-Leibler divergence to derive a bound.
\begin{align*}
...
Substituting  \eqref{eq:gmm_p} and \eqref{eq:gmm_q} in \eqref{eq:ELBO}
\begin{align*}
...
\end{align*}
I shall expand these terms separately in equations  (\ref{eq:ELBO:1}), (\ref{eq:ELBO:2}), (\ref{eq:ELBO:3F}), (\ref{eq:ELBO:4}) and (\ref{eq:ELBO:5}), using the following well known result--e.g. \cite[(5.7.5)]{fisz1963probability}.
...
\begin{align*}
	...
	&+ \underbrace{\sum_{i=1}^{n}\Expectation_q\big[\log{p(x_i\mid c_i,\mu);\phi_i,m,s^2}\big]}_\text{$= -\frac{1}{2} \sum_{i=1}^{n} \sum_{j=1}^{n}c_{ij} \big[s_j^2 + (x_i-m_j)^2 \big]$ from \eqref{eq:ELBO:3F}}\\
	...
	=&- \frac{1}{2 } \sum_{k=1}^{K} \big(s_k^2 +  m_k^2\big) -\frac{1}{2} \sum_{i=1}^{n} \sum_{j=1}^{n}c_{ij} \big[s_j^2 + (x_i-m_j)^2 \big] \numberthis \label{eq:ELBO:CAVI}
\end{align*}
