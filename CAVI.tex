...
If we combine (\ref{eq:ELBO:31}) with (\ref{eq:ELBO:32F}), (\ref{eq:ELBO:3}) becomes

\begin{align*}
	\Expectation_q\big[\log{p(x_i\mid c_i,\mu);\phi_i,m,s^2}\big] =& -\frac{1}{2}m_k -\frac{1}{2} \sum_{i=1}^{n} c_{ik} \big[s_k^2 + (x_k+m_k)^2 \big] \numberthis \label{eq:ELBO:3F}
\end{align*}
