...
def get_log_likelihood(xs, mu, Sigma, alpha,K=3):
    '''
    ...
    Returns:
        Log of likelihood
    '''
    return -0.5 * ((xs[:,None] - mu)**2 * alpha/Sigma).sum() - 0.5*(alpha*np.log(Sigma)).sum()

def maximize_likelihood(xs, mu=np.zeros((1)), Sigma=np.ones((1)), alpha=np.ones((1)), K=3, N=250,
    ...
    Parameters:
        ...
        Sigma      Starting values for Variances
    ...
    def e_step(mu, Sigma, alpha):
        ...
        '''
        w = np.exp(-0.5 *(xs[:,None]-mu)**2 /Sigma) *alpha/np.sqrt(Sigma)
    ...
    for i in range(N):
        likelihoods[i] = get_log_likelihood(xs,mu,Sigma,alpha,K=K)
...
if __name__ == '__main__':
    ...
    L, _, mu, Sigma = maximize_likelihood(xs,mu=mu0,Sigma=np.ones((K)),alpha=np.ones((K))/K,K=K,N=args.N)
